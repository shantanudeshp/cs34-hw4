...
TEST(OSMTest, SimpleNodeTest){
    ...
    auto TempNode3 = StreetMap.NodeByID(3);
    EXPECT_EQ(TempNode1,TempNode2);
    ASSERT_TRUE(bool(TempNode1));
    EXPECT_FALSE(bool(TempNode3));
    EXPECT_EQ(TempNode1->ID(),1);
    EXPECT_EQ(TempNode1->AttributeCount(),0);
    EXPECT_EQ(TempNode1->GetAttributeKey(0),"");
    EXPECT_EQ(TempNode1->GetAttribute(""),"");
    EXPECT_EQ(TempNode1->Location(),CStreetMap::SLocation(38.5,-121.7));
    ...
    EXPECT_EQ(TempNode1->AttributeCount(),0);
    EXPECT_EQ(TempNode1->Location(),CStreetMap::SLocation(38.5,-121.71));
}
...
TEST(OSMTest, SimpleWayTest){
    ...
    EXPECT_EQ(StreetMap.NodeCount(),2);
    auto TempNode1 = StreetMap.NodeByIndex(0);
    EXPECT_EQ(TempNode1->GetAttributeKey(0),"foot");
    EXPECT_EQ(StreetMap.WayCount(),1);
    auto TempWay = StreetMap.WayByIndex(0);
    auto TempWay2 = StreetMap.WayByID(3);
    auto TempWay3 = StreetMap.WayByID(9);
    ASSERT_TRUE(bool(TempWay));
    EXPECT_FALSE(bool(TempWay3));
    EXPECT_EQ(TempWay,TempWay2);
    EXPECT_EQ(TempWay->ID(),3);
...
TEST(OSMTest, WayTest){
    ...
    EXPECT_EQ(TempWay->GetAttributeKey(0),"oneway");
    EXPECT_EQ(TempWay->GetAttribute(""),"");
}

TEST(OSMTest, ErrorTet){
    std::vector<std::string> ErrorStrings = {   "<?xml version='1.0' encoding='UTF-8'?>"
                                                "<osm version=\"0.6\" generator=\"osmconvert 0.8.5\">"
                                                "<node id=\"1\" lat=\"38.5\" lon=\"-121.7\">"
                                                ...
                                                "<?xml version='1.0' encoding='UTF-8'?>"
                                                "<osm version=\"0.6\" generator=\"osmconvert 0.8.5\">"
                                                "<node id=\"foobar\" lat=\"38.5\" lon=\"-121.7\" />",
                                                "<?xml version='1.0' encoding='UTF-8'?>"
                                                "<osm version=\"0.6\" generator=\"osmconvert 0.8.5\">"
                                                "<node id=\"1\" lat=\"38.5\" lon=\"-121.7\">"
                                                "<tag k=\"foot\" v=\"yes\"/>"
    ...
    for(auto &ErrorString: ErrorStrings){
        auto InStream = std::make_shared<CStringDataSource>(ErrorString + std::string(1000,' '));
        auto Reader = std::make_shared<CXMLReader>(InStream);
        COpenStreetMap StreetMap(Reader);   
            
        EXPECT_EQ(StreetMap.WayCount(),std::string::npos == ErrorString.find("way") ? 0 : 1);
