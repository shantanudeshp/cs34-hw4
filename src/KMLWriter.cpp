...
struct CKMLWriter::SImplementation{
    ...
    bool StartTag(const std::string &name, const TAttributes &attributes){
        if(OutputIndent()){
            SXMLEntity Entity;
    ...
    bool CreatePoint(const std::string &name, const std::string &desc, const std::string &stylename, CStreetMap::SLocation point){
        if(DPointStyles.count(stylename) && 
            ...
            IndentedData({std::to_string(point.DLongitude) + "," + std::to_string(point.DLatitude)});
    ...
    bool CreatePath(const std::string &name, const std::string &stylename, const std::vector< CStreetMap::SLocation > &points){
        std::vector<std::string> PointStrings;
        PointStrings.reserve(points.size());
        for(auto &Point : points){
            PointStrings.push_back(std::to_string(Point.DLongitude) + "," + std::to_string(Point.DLatitude));
        }
        if(DLineStyles.count(stylename) && 
            StartTag(DPlacemarkTag,{}) && 
            StartTagDataEndTag(DNameTag,name) && 
...
bool CKMLWriter::CreatePoint(const std::string &name, const std::string &desc, const std::string &stylename, CStreetMap::SLocation point){
    return DImplementation->CreatePoint(name,desc,stylename,point);
}

bool CKMLWriter::CreatePath(const std::string &name, const std::string &stylename, const std::vector< CStreetMap::SLocation > &points){
