...
class CKMLTranslator{
    private:
        std::unordered_map<CStreetMap::TNodeID,CStreetMap::SLocation> DNodeIDToLocation;
        std::unordered_map<CStreetMap::TNodeID,CBusSystem::TStopID> DNodeIDToStopID;
        std::unordered_map<TNodeIDPair,std::vector<CStreetMap::SLocation>,SNodeIDPairHasher> DBusSegmentToLocations;
...
CKMLTranslator::CKMLTranslator(std::shared_ptr<CStreetMap> map, std::shared_ptr<CDSVReader> stops, std::shared_ptr<CDSVReader> buspaths){
    ...
    if(buspaths->ReadRow(TempRow)){
        ...
        while(buspaths->ReadRow(TempRow)){
            ...
            std::vector<CStreetMap::SLocation> LocationList;
...
bool CKMLTranslator::TranslateFile(const std::string &filename){
    ...
    CStreetMap::SLocation LasSLocation;
    std::vector<CStreetMap::SLocation> SubPathLocations;
    ...
    for(auto &PathStep : ParsePathFile(TripReader)){
        ...
        if(CurrentNodeID == CStreetMap::InvalidNodeID){
            Description = std::string("Start Point\nNode ID: ") + std::to_string(NodeID) + "\nLatitude: " + std::to_string(Location.DLatitude)+ "\nLongitude: " + std::to_string(Location.DLongitude);
            ...
            if(Mode != LastMode){
                ...
                SubPathLocations.push_back(LasSLocation);
            }
            if(Mode == "Bus"){
                if(Mode != LastMode){
                    Description = std::string("Bus Stop\nStop ID: ") + std::to_string(DNodeIDToStopID[CurrentNodeID]) + "\nLatitude: " + std::to_string(LasSLocation.DLatitude)+ "\nLongitude: " + std::to_string(LasSLocation.DLongitude);
                    KMLWriter.CreatePoint("Bus Stop",Description,PointStyle,LasSLocation);
                }
    ...
    KMLWriter.CreatePoint("End Point",Description,PointStyle,LasSLocation);
